...
package body follow_path is
   ...
   begin
      ...
      Ada.Text_IO.Put_Line
        (" --> Wheel Angle is: " &
         Float'Image (wheel_angle) &
         " Steering Angle is " &
         Float'Image (steering_angle));
   ...
   begin
      ...
      --  else
      return (Float (sumx) / Float (pixel_count) / Float (cols'Last) - 0.5) *
        camera_fov;
      --  end if;
   ...
   begin
      ...
      if check_distance (ds_values) then
         ...
         right_speed := speed;
         line_angle  :=
           filter_angle (process_camera_image (img, line_plan (1)));

         if
           (filter_angle (process_camera_image (img, line_plan (2))) /=
            UNKNOWN)
         then
            line_angle :=
              filter_angle (process_camera_image (img, line_plan (2)));
           ...
           (filter_angle (process_camera_image (img, line_plan (3))) /=
            UNKNOWN)
         then
            line_angle :=
              filter_angle (process_camera_image (img, line_plan (3)));
           ...
           (filter_angle (process_camera_image (img, line_plan (4))) /=
            UNKNOWN)
         then
            line_angle :=
              filter_angle (process_camera_image (img, line_plan (4)));
            ...
            line_angle :=
              filter_angle (process_camera_image (img, line_plan (5)));
            line_plan (4) := (-1, -1, -1);
         end if;
